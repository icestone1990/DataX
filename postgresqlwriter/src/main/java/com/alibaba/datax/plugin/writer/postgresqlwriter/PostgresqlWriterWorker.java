...
public class PostgresqlWriterWorker implements Callable<Long> {
	private static int MaxCsvSize = 4194304;
	private static final Logger LOG = LoggerFactory.getLogger(PostgresqlWriterWorker.class);

	private PostgresqlWriterTask task = null;
	...
	public PostgresqlWriterWorker(PostgresqlWriterTask task, String copySql, LinkedBlockingQueue<byte[]> queue) throws IOException {
		this.task = task;
...
//		changeCsvSizelimit(connection);
	...
	@Override
	public Long call() throws Exception {
		Thread.currentThread().setName("PostgresqlWriterWorker");
