...
public class GreenplumWriterTask extends CommonRdbmsWriter.Task {
	private static final Logger LOG = LoggerFactory.getLogger(GreenplumWriterTask.class);
...
//		ExecutorService threadPool;
		...
		try {
			...
			for (int i = 0; i < numProcessor; i++) {
				cs.submit(new GreenplumWriterProcessor(this, this.columnNumber, resultSetMetaData, recordQueue, dataQueue));
			}

			for (int i = 0; i < numWriter; i++) {
				cs.submit(new GreenplumWriterWorker(this, sql, dataQueue));
			}
